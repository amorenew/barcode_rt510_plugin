...
import com.android.hdhe.barcode.reader.BarcodeReader;
import com.android.hdhe.barcode.readerInterface.TagModel;
import com.handheld.BarcodeLongerDemo.EPC;
import com.handheld.BarcodeLongerDemo.MyApplication;
...
public class BarcodeHelper {
    ...
    public void init(BarcodeListener barcodeListener) {
    ...
    public void connect() {
        try {
            manager = BarcodeReader.getInstance();
            new Timer().schedule(new TimerTask() {
                public void run() {
                    if (manager != null && manager.setOutputPower((short) signalPowerLevel)) {
                        connectFlag = true;
                        myApp.setmanager(manager);
                        barcodeListener.onConnect(true, signalPowerLevel);
                    } else {
                        connectFlag = false;
                        barcodeListener.onConnect(false, -1);
                    }
                }
            }, 100);
        } catch (Exception e) {
    ...
    private void addToList(final List<EPC> list, final String epc) {
        ...
        barcodeListener.onRead(jsonArray.toString());
